...
.controller('ProfileCtrl', function($scope, $ionicHistory, $state, $ionicScrollDelegate, loggedUser, user, posts) {
  $scope.$on('$ionicView.afterEnter', function() {
    $ionicScrollDelegate.$getByHandle('profile-scroll').resize();
  });

  $scope.currentUserId = user.idusuario;
  $scope.user = user;

  $scope.myProfile = loggedUser.idusuario == user.idusuario;
  ...
  $scope.getUserPosts = function(){
    ...
    $state.go('app.profile.posts', {userId: user.idusuario});
  };
})
...
.controller('CategoryFeedCtrl', function($scope, _, FeedService, $ionicModal, $stateParams, loggedUser, feed, category) {
  $scope.loggedUser = loggedUser;
  $scope.cards = feed.posts;
  $scope.current_category = category;
  //$scope.current_subcategory = subcategory;
  ...
  $ionicModal.fromTemplateUrl('views/app/post/escribir-publicacion.html', {
    scope: $scope,
    animation: 'slide-in-up'
  }).then(function(modal) {
    $scope.escribir_publicacion = modal;
  });

  $scope.escribirPublicacion = function(){
    $scope.escribir_publicacion.show();
  };
...
// SIDE MENU
.controller('Menu', function($scope, $http, loggedUser, categorias){
...
.controller('BrowseCtrl', function($scope, $stateParams, $http, subcategorias) {
  
  $scope.idcategoria    = subcategorias.idcategoria;
  $scope.categoria      = subcategorias.categoria;
  $scope.subcategorias  = subcategorias.subcategorias;
...
.controller('SettingsCtrl', function($scope, $ionicModal, $state, localStorageService) {
